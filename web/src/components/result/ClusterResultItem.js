            ...
            if (!!childLisbs && childLisbs.length > 0) {
                ...
                var compomentArray = childLisbs.reduce((arr, item, index)=> {
                    if (index + 1 == childLisbs.length && isOddNumber) {//最后一行只有一个子元素
                        arr.push(<div className="col-sm-12"><ChildClusterResultItem libId={libId} type={type}
                                                                                    degree={item.degree}
                                                                                    choren={item.choren}/></div>);
                    } else {
                        arr.push(<div className="col-sm-6"><ChildClusterResultItem libId={libId} type={type}
                                                                                   degree={item.degree}
                                                                                   choren={item.choren}/></div>);
                    }
